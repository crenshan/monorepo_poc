...
│   └── ds-mcp/   @mono/ds-mcp — an MCP server exposing the design system to AI agents
...
- **`packages/ds-mcp`** — an MCP server that lets AI agents list/search `@mono/ui` components, read prop details, look up design tokens, and pull curated usage examples.
...
| `pnpm tokens`   | Regenerates `packages/ui/src/tokens.css`, `packages/ui/generated/tokens.json`, and `packages/ui/generated/components.json` |
...
- There's deliberately no Turborepo, Nx, or Changesets here — with a few small packages, that tooling would be ceremony without payoff. If the workspace grows significantly, that's worth reconsidering.
...
2. As CSS custom properties (`packages/ui/src/tokens.css`, regenerated via `pnpm tokens`) — component CSS reads spacing, color, typography, and radii from these variables. Raw values are still fine for layout mechanics (percentages, viewport units, `fr`, `flex`, container max-widths, 1px borders).
