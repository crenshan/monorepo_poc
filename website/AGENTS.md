...
- Always check the component barrel (`packages/ui/src/components/index.ts`) before writing or updating markup in a view. If a primitive exists, use it. This intentionally departs from root `AGENTS.md`'s MCP-first default: a controlled 3-run test found the `ds`-server-first workflow increased view-authoring tool calls and tokens without a fidelity gain here — revisit if the design system or its MCP tooling changes materially. The `ds` server's `new-view` prompt still walks through the full MCP-first workflow as an explicit, opt-in demo — this rule only governs the unprompted default.
