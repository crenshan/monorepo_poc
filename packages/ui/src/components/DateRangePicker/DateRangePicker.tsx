 ...
 * pick a bounded date range (e.g. filtering a report by date).
 *
 * @category User Input
 *
 * @example
 ...
 *
 * @remarks
 * The component is controlled: it always reflects `value` and reports changes via
 * `onChange`. Each field's bounds constrain the other to keep the range valid — the
 * start field's `max` is the selected end date (or the `max` prop), and the end field's
 * `min` is the selected start date (or the `min` prop). The two fields render as an
